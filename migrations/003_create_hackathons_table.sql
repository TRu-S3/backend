...
COMMENT ON COLUMN hackathons.id IS 'ハッカソンID（主キー）';
COMMENT ON COLUMN hackathons.name IS 'ハッカソン名';
COMMENT ON COLUMN hackathons.description IS 'ハッカソンの説明';
COMMENT ON COLUMN hackathons.start_date IS '開始日時';
COMMENT ON COLUMN hackathons.end_date IS '終了日時';
COMMENT ON COLUMN hackathons.registration_start IS '参加登録開始日時';
COMMENT ON COLUMN hackathons.registration_deadline IS '参加登録締切日時';
COMMENT ON COLUMN hackathons.max_participants IS '最大参加者数（0は無制限）';
COMMENT ON COLUMN hackathons.location IS '開催場所';
COMMENT ON COLUMN hackathons.organizer IS '主催者';
COMMENT ON COLUMN hackathons.contact_email IS '連絡先メールアドレス';
COMMENT ON COLUMN hackathons.prize_info IS '賞品・賞金情報';
COMMENT ON COLUMN hackathons.rules IS 'ルール・規則';
COMMENT ON COLUMN hackathons.tech_stack IS '推奨技術スタック（JSON形式）';
COMMENT ON COLUMN hackathons.status IS 'ステータス（upcoming/ongoing/completed/cancelled）';
COMMENT ON COLUMN hackathons.is_public IS '公開/非公開フラグ';
COMMENT ON COLUMN hackathons.banner_url IS 'バナー画像URL';
COMMENT ON COLUMN hackathons.website_url IS '公式ウェブサイトURL';

COMMENT ON TABLE hackathon_participants IS 'ハッカソン参加者テーブル';
COMMENT ON COLUMN hackathon_participants.id IS '参加者ID（主キー）';
COMMENT ON COLUMN hackathon_participants.hackathon_id IS 'ハッカソンID（外部キー）';
COMMENT ON COLUMN hackathon_participants.user_id IS 'ユーザーID（外部キー）';
COMMENT ON COLUMN hackathon_participants.team_name IS 'チーム名';
COMMENT ON COLUMN hackathon_participants.role IS '役割';
COMMENT ON COLUMN hackathon_participants.registration_date IS '参加登録日時';
COMMENT ON COLUMN hackathon_participants.status IS 'ステータス（registered/confirmed/cancelled/disqualified）';
COMMENT ON COLUMN hackathon_participants.notes IS '参加時のメモ・自己紹介';